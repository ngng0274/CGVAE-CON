...
class DenseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        # put in guassian noise
        self.placeholders['guassian_noise_std']=tf.placeholder(tf.float32, [], name='guassian_noise_std') # number
        # put in front of l2 norm loss
        self.placeholders['l2_norm_lambda']=tf.placeholder(tf.float32, [], name='l2_norm_lambda') # number
    ...
    def augmented_by_gaussian_noise(self):
        std = self.placeholders['guassian_noise_std']
        noise = tf.random_normal(shape=tf.shape(self.ops['z_sampled']), mean=0.0, stddev=std, dtype=tf.float32)
        return self.ops['z_sampled'] + noise
    ...
    def construct_loss(self):
        ...
        # l2 norm for z_sampled 1, z_sampled 2 and z_sampled
        l2_norm_lambda = self.placeholders['l2_norm_lambda']
        #nor_z_sampled = tf.nn.l2_normalize(self.ops['z_sampled'], 2)
        #nor_z_sampled_1 = tf.nn.l2_normalize(self.ops['z_sampled_1'], 2)
        #nor_z_sampled_2 = tf.nn.l2_normalize(self.ops['z_sampled_2'], 2)
        self.ops['l2_norm'] = tf.nn.l2_loss(self.ops['z_sampled'] - self.ops['z_sampled_1']) + tf.nn.l2_loss(self.ops['z_sampled'] - self.ops['z_sampled_2']) + tf.nn.l2_loss(self.ops['z_sampled_1'] - self.ops['z_sampled_2'])

        self.ops['total_l2_norm'] = l2_norm_lambda *self.ops['l2_norm'] 
        ...
        return tf.reduce_mean(self.ops["edge_loss"] + self.ops['node_symbol_loss'] + \
                              ...
                              l2_norm_lambda *self.ops['l2_norm'])\
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                ...
                self.placeholders['guassian_noise_std']: self.params['guassian_noise_std'],
                self.placeholders['l2_norm_lambda']: self.params['l2_norm_lambda'],
